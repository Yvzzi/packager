...
$option = Cli::getCliOption($argv, ["u:", "p:", "z", "o:", "m:", "c", "t:", "h", "s"]);
...
assertParams(!$option->option("u") || !$option->option("p") || !$option->option("c"));
if ($option->option("c")) {
...
} elseif ($option->option("u")) {
    if ($option->option("o"))
        $output = FileIO::getAbsolutePath($option->option("o"));
    else {
        $output = basename($option->option("u"));
        $index = strrpos($output, ".");
        $output = substr($output, 0, $index);
    }
    PharBuilder::unphar(FileIO::getAbsolutePath($option->option("u")), $option->option("z") ? $output . ".zip" : $output, $option->option("z"));
} else {
    assertParams(!empty($option->option("p")));
    $ignore = [];
    $projectDir = FileIO::getAbsolutePath($option->option("p"));
    if ($option->option("m")) {
